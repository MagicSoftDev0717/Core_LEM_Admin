...
export default function BasicTableOne({ name }: { name: string }) {
    ...
    useEffect(() => {
        ...
        if (!name) return; // Prevent execution if id is not yet available
        async function fetchLeads() {

            try {
                const response = await fetch(`/api/details_ld_a/${name}`);
