...
export async function GET(req: Request) {
  try {
    ...
    if (selectedSts !== "all") {
      whereCondition.status = selectedSts;
    }
    ...
    teacher_data = await prisma.teacher.findMany({
    ...
    const totalTeacher = await prisma.teacher.count({ where: whereCondition });
    const totalActive = await prisma.teacher.count({ where: { status: "active" }});

    return NextResponse.json({ teacher_data, totalTeacher, totalActive });
  } catch (error) {
