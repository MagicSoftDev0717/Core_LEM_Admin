...
export async function GET(req: Request) {
  try {
    ...
    const whereCondition: Prisma.ComconWhereInput = {}; 
    ...
    comcon_data = await prisma.comcon.findMany({
    ...
    const totalComcon = await prisma.comcon.count({ where: whereCondition });

    return NextResponse.json({ comcon_data, totalComcon });
  } catch (error) {
